...
//                 <TableCell>
//                   <Link to={{ pathname: "/edicaocliente", state: { client } }}>
...
import { 
  ...
  Paper, TextField, Button, IconButton, Pagination, Dialog, 
...
function ListagemClientes() {
  ...
  const handleEdit = async (cpf) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://servpro.onrender.com/api/Cliente/${cpf}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDelete = async () => {
    try {
      if (selectedClient && selectedClient.CPF) {
        const token = localStorage.getItem('token'); 
        await axios.delete(`https://servpro.onrender.com/api/Cliente/${selectedClient.CPF}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
